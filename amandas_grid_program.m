function varargout = amandas_grid_program(varargin)
% AMANDAS_GRID_PROGRAM MATLAB code for amandas_grid_program.fig
%      AMANDAS_GRID_PROGRAM, by itself, creates a new AMANDAS_GRID_PROGRAM or raises the existing
%      singleton*.
%
%      H = AMANDAS_GRID_PROGRAM returns the handle to a new AMANDAS_GRID_PROGRAM or the handle to
%      the existing singleton*.
%
%      AMANDAS_GRID_PROGRAM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in AMANDAS_GRID_PROGRAM.M with the given input arguments.
%
%      AMANDAS_GRID_PROGRAM('Property','Value',...) creates a new AMANDAS_GRID_PROGRAM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before amandas_grid_program_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to amandas_grid_program_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help amandas_grid_program
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @amandas_grid_program_OpeningFcn, ...
                   'gui_OutputFcn',  @amandas_grid_program_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before amandas_grid_program is made visible.
function amandas_grid_program_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to amandas_grid_program (see VARARGIN)

% Choose default command line output for amandas_grid_program
...
% UIWAIT makes amandas_grid_program wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = amandas_grid_program_OutputFcn(hObject, eventdata, handles) 
...
%     yidx = 1 + round((centroids_new(:,2) - minvals(2)) ./ rangevals(2) * (grid-1));
    ...
    density = accumarray([yidx, xidx], 1, [grid,grid])/length(heatmap_struct);  %note y is rows, x is cols
